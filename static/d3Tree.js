...
d3.json(params.dataLoadUrl, function(data) {
  ...
  params.pristinaData = JSON.parse(JSON.stringify(data.data));
...
function drawOrganizationChart(params) {
  ...
  dynamic.rootNodeLeftMargin = attrs.width / 2-attrs.nodeWidth/2;
  ...
  var zzz = attrs.width/2 -attrs.nodeWidth/2;
  var svg = d3.select(attrs.selector)
    ...
    .call(zoomBehaviours)
    .append("g")
    .attr("transform", "translate(" + zzz + "," + 20 + ")");
    ...
    var collapsibleRects = collapsiblesWrapper.append("rect")
      ...
      .attr("width", function(d) {
        ...
        return 0;
			...
			var eeefff=
				collapsiblesWrapper.append("svg:image")
...
//    		if(v.nodesType==41){
//    			xlink:href="./static/users.png"
////    			eeefff.href="./static/users.png"
////    			attrs.e="./static/users.png"
//    		}else{
////    			eeefff.href=""
//    		}
////    		return eeefff
//    })
      .attr('href',function(v){
      	if(v.nodesType==41){
      		return "./static/users.png"
      	}else{
      		return ""
      	}
      })
//			console.log(eeefff)
//			for(var i=0;i<eeefff.length;i++){
//				eeefff[i].href.animVal=""
//			}
    ...
    nodeGroup.append("text")
      ...
      .text(function(d) {
//    	var s=d.name,reg=/.{7}/g,rs=s.match(reg);  
//       if(d.name.length<6){  
//          return d.name.trim();  
//       }else{  
...
//    	console.log(d.positions)
      	if(d.positions!=null){
      		var l = d.positions.length; 
					var blen = 0; 
					for(i=0; i<l; i++) { 
							if ((d.positions.charCodeAt(i) & 0xff00) != 0){ 
								blen ++; 
							} 
								blen ++;
					}
					if(blen>20){
						d.positions=d.positions.substring(0,10)+'...'
					}
      	
//      var position =  d.positions.substring(0,27);
//   	if(position.length<d.positions.length){
...
//  }
    ...
    nodeGroup.append("svg:image")
      ...
      .attr("xlink:href", function(v) {
      	if(v.imgUrl==null && v.nodesType==40){
    			v.imgUrl="./static/department.png"
      	}
	    	if(v.imgUrl==null && v.nodesType==41){
	    			v.imgUrl="./static/departmentmorw.png"
	    	}
         return v.imgUrl;
      })
...
//     }
    ...
    function tooltipContent(item) {
    	...
    	var l = item.name.length; 
					var blen = 0; 
					for(i=0; i<l; i++) { 
							if ((item.name.charCodeAt(i) & 0xff00) != 0){ 
								blen ++; 
							} 
								blen ++;
					}
      ...
      var strVar = "";
      strVar += "  <div class=\"customTooltip\" id='idx0'>";
      strVar += "    <!--";
      ...
      strVar += "-->";
	    strVar += "    <div class=\"profile-image-wrapper\" style='background-image: url(" + item.imgUrl + ")'>";
//			strVar += "    		<src class=\"profile-image-wrapper\" src=\"..\/static\/+ 'item.imgUrl' +\">";
     // strVar += "          <img class=\"imgAdd\" src=\"..\/static\/add.png\" @click='newClick'>";
     // strVar += "          <img class=\"imgAdd\" src=\"..\/static\/add.png\" onclick='newClick1()'>";
      strVar += "    <\/div>";
      strVar += "    <div class=\"tooltip-hr\"><\/div>";
      strVar += "    <div class=\"tooltip-desc\">";
      strVar += "      <a class=\"name\" href='' > " + item.name  + "<\/a>";
      strVar += "      <p class=\"position\">" + flags.positions + " <\/p>";
      strVar += "      <p class=\"area\">" + item.phone + " <\/p>";
	  	strVar += "      <p class=\"emails\">"+ item.email +"</p>";
      strVar += "";
      // strVar += "      <p class=\"office\">""<\/p>";
  ...
  function tooltipHoverHandler(d) {
 	var bt0= document.getElementById("bt02");
 	var bt5=document.getElementById("bt05");
 	var bt9=document.getElementById("bt09");
    flags=d
    ...
    var tool_tip = document.getElementById("idx0");
    
  	if(false == bt0.parentElement.isSameNode(tool_tip)){
  		tool_tip.appendChild(bt0);
  		tool_tip.appendChild(bt5);
  		tool_tip.appendChild(bt9);
	}
  	if(d.nodesType=="41"){
  	...
  	console.log(d)
  	//判断是否有子节点
	if((null != d.children && d.children.length>0) || (null != d._children && d._children.length>0)){
  ...
  function wrap(text, width) {
    text.each(function() {
      var text = d3.select(this),
        ...
        line = [],
        lineNumber = 0,
        lineHeight = text.node().getBoundingClientRect().height, // ems
        x = text.attr("x"),
        y = text.attr("y"),
        ...
        .attr("y", y)
      	.attr("dy", 0 + "em");
					...
					var blen = 0; 
					for(i=0; i<l; i++) { 
							if ((words[0].charCodeAt(i) & 0xff00) != 0){ 
								blen ++; 
							} 
								blen ++;
					}
					if(blen>16){
						words[0]=words[0].substring(0,7)+'...'
					}
					
      while (word = words.pop()) {
        line.push(word);
        const dash = lineNumber > 10 ? '-' : '';
